...
class Ddlogin extends Backend
{
	...
	public function setkey(){
		...
		for($n = 0;$n<$countdepartment;$n++){
			$departmentid = $countdepartment[$n]['id'];
			$userlist = $this->getuser($access_token,$departmentid);
			
			$departid = get_object_vars();
			if($n == 0){
				$first = $userlist;
			}else{
				$first = array_intersect($first,$userlist);
			}
		}
		dump($first);
			// $departmentid = $val['id'];
			// dump($departmentid['id']);
			// $lastinf = $this->getuser($access_token,$departmentid);
	}
	public function getuser($access_token,$departmentid){
