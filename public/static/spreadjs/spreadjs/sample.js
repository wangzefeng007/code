...
console.log(req);
console.log(decodeURIComponent(req.name))
...
function syncSheetPropertyValues() {
    ...
    if (req.type == 1) {
        console.log(req)
        syncProtectSheetRelatedItems(sheet, true);
    } else {
    ...
    if (req.id) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.msg == 1 && res.code) {
                    $('.none_data').hide()
                    $('.have_data').show()
                    let json = JSON.parse(res.code.value)
                    // 渲染数据到表格
                    importJson(json);
                } else {
                    // alert(res.msg)
...
$(function () {
    ...
    $('.pop-up-but').click(function () {
        console.log('选择的是' + select, select1)
        $('.pop-up').hide()
    })
...
function preview(x) {
    ...
    if (req.id) {
        // 数据预览
        $.ajax({
            url: "getexceldataval?id=" + req.id + "&liucid=" + req.liucid,
            type: "GET",
            dataType: "json",
            data: {
...
function attachToolbarItemEvents() {
    ...
    $("#doExport").click(function () {
        if (saveConfirm) {
            ...
            // let sheet = new GC.Spread.Sheets.Print;
            // sheet.setRowCount(3, GcSpread.Sheets.SheetArea.viewport);
            // sheet.setColumnCount(3, GcSpread.Sheets.SheetArea.viewport);

            // sheet.isPaintSuspended(true);
            // //set value
            // for (var r = 0, rc = sheet.getRowCount(); r < rc; r++) {
            //     for (var c = 0, cc = sheet.getColumnCount(); c < cc; c++) {
            //         sheet.setValue(r, c, r + c);
            //     }
            // }
            // sheet.isPaintSuspended(false);
            // let instance = new GC.Spread.Sheets.Print.PrintInfo()
            // var returnValue; // Type: PaperSize
            // returnValue = instance.paperSize(value);
            // console.log(returnValue)
            // instance.headerLeft("Header Left");
            // instance.paperSize(new GcSpread.Sheets.PaperSize(GcSpread.Sheets.PaperKind.A4));
            ...
            //打印
            //设置打印
            // let spreads = new GC.Spread.Sheets.Workbook(document.getElementById("ss"), { sheetCount: 1 });
            // initSpreads(spreads);
            setPrintInfo()
            spread.print(0)
...
function setPrintInfo() {
    ...
    // var spread = GC.Spread.Sheets.findControl("formulabox");
    var sheet = spread.getActiveSheet(),
    ...
    printInfo.showRowHeader('hide');
    printInfo.showColumnHeader('hide');
    printInfo.showBorder(false);

    //缩放系数
    // printInfo.zoomFactor(0);
...
function importSpreadFromExcel(file, options) {
    ...
    excelIO.open(file, function (json) {
        ...
        for (let key in json.sheets) {
            keyData.push(json.sheets[key])
            console.log(keyData)
            if (keyData.length >= 3) {
                json.sheets = {}
                json.sheets[keyData[0].name] = keyData[0]
                json.sheets[keyData[2].name] = keyData[2]
            }
        }
        importJson(json);
    }, function (e) {
...
function getFileName() {
    ...
    return decodeURIComponent(req.name)
}
...
// 取到选中单元格对应的参数
    ...
    var sheet = spread.getActiveSheet();
    console.log(sheet.getSelections()[0])
    console.log(JSON.stringify(sheet.getArray(sheet.getSelections()[0].row,
    sheet.getSelections()[0].col,
    sheet.getSelections()[0].rowCount,
    sheet.getSelections()[0].colCount)))
    ...
    // sheet.setValue(0,0,"垂直合并单元格");
    // sheet.setValue(4,0,"水平合并单元格");
    ...
    //         var span = spans[i];
    //         //判断
    //         if(span.rowCount > span.colCount) {
    //             // 输入垂直合并单元格信息
    //             console.log(span);
    //         }
    //     }
    ...
    // // 保存到服务器  text 是数据    id为 req.id    value为  text
    // $.ajax({
    //     url: "example/excel/save?id=" + req.id,//json文件位置
    //     type: "POST",//请求方式为get
...
// function getData(){
//     $.ajax({
//         url: "example/excel/save?id=1",//json文件位置
//         type: "POST",//请求方式为get
//         dataType: "json", //返回数据格式为json
//         data: {
